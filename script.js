/* ============================================================
   Web OS 2.0 — Desktop Environment
   V2 architecture:
     - Storage  : namespaced localStorage (webos.*) with safe JSON
     - VFS      : virtual file system (JSON tree) with timestamps
     - Trash    : recycle bin persisted under webos.trash
     - WM       : window manager — drag, 8-direction resize,
                  maximize/restore, minimize, z-order, persisted geometry
     - Apps     : central registry — icons, start menu, taskbar,
                  search and shortcuts all read from it
     - NotifCenter : persistent notification store + toasts + badge
     - Shortcuts: global keyboard handling (Ctrl+K, Alt+Tab, Alt+F4…)
     - Clock/CalendarPanel, StartMenu grid + search, Desktop menus
   ============================================================ */
"use strict";
...
const VFS = {
  ...
  load() {
    ...
    if (!this.data) {
      ...
      this.save(); // persist the default tree on first boot
    }
  },
        ...
        Documents: {
          ...
          children: {
            "welcome.txt": { type: "file", name: "welcome.txt", created: t, modified: t,
              content: "Welcome to Web OS 2.0!\n\nThis file lives in the virtual file system and is saved in your browser's localStorage. V2 adds copy/cut/paste, sorting, search, breadcrumbs and a Recycle Bin." },
  ...
  // Hard delete (bypasses trash) — used by "delete permanently"
  remove(path, name) {
    const folder = this.resolve(path);
...
const Trash = {
  ...
  // Move an entry (file OR folder subtree) from VFS into the trash
  put(path, name) {
  ...
  restore(index) {
    ...
    if (folder.children[item.name]) return false; // name collision — do not overwrite
...
/* ---------------- Toasts (visual layer of notifications) ---------------- */
const Notify = {
...
const Apps = {}; // name -> { name, title, icon, width, height, desktop?, mount(body, win) }
...
const WM = {
  layer: null,
  zTop: 100,
  windows: {}, // id -> { id, app, el, preMax, minimized, taskBtn }
  MINW: 280,
  MINH: 180,
  ...
  open(appName) {
    ...
    // Single instance per app: focus / restore instead of duplicating.
    if (this.windows[id]) {
    ...
    // Geometry: restore the user's last size/position for this app, else cascade
    ...
    // Bring to front on any pointer-down inside the window
  ...
  // Alt+Tab: cycle focus through open (non-minimized) windows
  cycle() {
    ...
    this.focus(open[0].id); // lowest z-index = was focused longest ago
  },
  ...
  /* Pure geometry math for resizing — unit-testable */
  ...
  /* --- 8-direction resize (edges + corners), V2 --- */
  enableResize(w) {
  ...
  /* Persist per-app window geometry so size is remembered */
  saveState() {
    const st = {};
    Object.values(this.windows).forEach(w => {
...
/* ---------------- Notes app (V2: search, pin, counts, timestamps) ---------------- */
registerApp({
  name: "notes",
  title: "Notes",
  icon: "📝",
...
registerApp({
  ...
  mount(body) {
    ...
    let cwd = [];
    let selected = null;
    let clipboard = null; // { mode: "copy"|"cut", from: [...], name }
...
/* ---------------- Calculator app (V2: scientific, memory, history) ---------------- */
registerApp({
  name: "calculator",
  title: "Calculator",
  icon: "🧮",
  ...
  mount(body) {
    ...
    function evaluateExpr(expr) {
      // Only digits, operators, parens, dots and scientific placeholders survive;
      // placeholders were already substituted with numeric literals.
    ...
    function press(k) {
      if (/^s\(|^c\(|^t\(|^l\(|^n\(/.test(k)) { // sci functions wrap current value
...
/* ---------------- Browser app (V2 simulator: history, bookmarks) ----------------
   Deliberately a simulator: real sites send X-Frame-Options headers that block
   iframes, so this app renders its own pages and opens real results externally. */
registerApp({
  name: "browser",
  title: "Browser",
  icon: "🌐",
  ...
  mount(body) {
    ...
    let stack = ["home"]; // navigation stack of page keys ("home" or a query)
    let idx = 0;
    ...
    function render() {
      ...
      // Search results page (simulated, links open in the real browser)
      content.innerHTML =
    ...
    function navigate(page) {
      // truncates forward history, like a real browser
...
/* ---------------- Settings app (V2: accents, style, icons, shortcuts) ---------------- */
const WALLPAPERS = {
...
const Settings = {
  defaults: {
    ...
    accent: "purple", winStyle: "glass", iconSize: "medium", showIcons: true
  },
...
/* ---------------- System Info app (V2: cards + storage bar) ---------------- */
registerApp({
  name: "sysinfo",
  ...
  mount(body) {
    function render() {
      ...
      const quota = 5 * 1024 * 1024; // typical localStorage quota ~5 MB
      ...
      body.innerHTML =
        '<div class="sysinfo-app">' +
          '<div class="sys-card"><h4>System</h4><table>' +
            "<tr><td>OS</td><td>Web OS 2.0</td></tr>" +
  ...
  mount(body) {
    body.innerHTML =
      '<div class="about-app">' +
        ...
        '<div style="text-align:center"><span class="version-tag">Version 2.0 — Evolution</span></div>' +
        ...
        "<p>V2 highlights: 8-direction resizable windows with remembered sizes, an advanced File Manager (copy/cut/paste, sort, search, breadcrumbs), a Recycle Bin, a Notification Center, a calendar panel, a browser simulator with history and bookmarks, a scientific calculator, global keyboard shortcuts, and deep personalization.</p>" +
...
const Clock = {
  timer: null,
  calMonth: null, // Date of the month being viewed
...
/* ---------------- Start menu (launcher grid + search) ---------------- */
const StartMenu = {
  el: null,
  input: null,
  ...
  init() {
    ...
    appsBox.innerHTML = "";
    Object.values(Apps).forEach(def => {
      const btn = document.createElement("button");
...
/* ---------------- Desktop: icons, context menu, trash badge ---------------- */
const Desktop = {
  init() {
    const box = document.getElementById("desktopIcons");
    box.innerHTML = "";

    // Desktop shortcuts: main apps + the Recycle Bin
    const entries = [
      ...["notes", "files", "calculator", "browser", "settings", "sysinfo"].map(n => ({ app: n })),
      { app: "trash", trash: true }
    ];

    entries.forEach(({ app, trash }) => {
      const def = Apps[app];
      if (!def) return;
      ...
      icon.innerHTML = '<div class="icon-box">' + def.icon +
        (trash ? '<span class="trash-badge" id="trashBadge" hidden></span>' : "") +
...
/* ---------------- Notification Center panel + Calendar panel wiring ---------------- */
const Panels = {
  init() {
...
const Shortcuts = {
  init() {
    document.addEventListener("keydown", e => {
      ...
      // Meta/Win key — start menu (browsers vary; best effort)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => NotifCenter.push("Welcome to Web OS 2.0",
    "Right-click the desktop, press Ctrl+K to search, and try the Recycle Bin.", "👋", 5000), 600);
});
